...
{
    [CreateAssetMenu(fileName = "BlenderGlobalImportSettings",
        menuName = "BlenderImporter/BlenderGlobalImportSettings")]
    public class BlenderImporterGlobalSettings : ScriptableObject
    {
        ...
        public static BlenderImporterGlobalSettings instance
        {
            get
            {
                ...
                {
                    var settings = CreateInstance<BlenderImporterGlobalSettings>();
        ...
        [Header("Console Logging")] public Color PythonConsoleLabelColor = new Color(0.22f, 1f, 0.0f);
        ...
        public string GetPythonScriptLocation()
        {
            ...
            Debug.LogWarning(
                $"Could not find blender script at {defaultLocation}. Searching for it in the Unity directory.");

            var files = System.IO.Directory.GetFiles(Application.dataPath, "*" + _pythonScriptName,
                System.IO.SearchOption.AllDirectories);

            if (files.Length > 0) _pythonScriptLocation = files[0];

            throw new System.Exception(
                $"Could not find {defaultLocation} in the project, unable to import blend files.");
        }
    }
            ...
            string[] movedAssets, string[] movedFromAssetPaths)
        {
            for (var index = 0; index < importedAssets.Length; index++)
            {
                ...
                if (importedAsset.Contains(".asset"))
                {
                    ...
                    {
                        var importSettings = (BlenderImporterGlobalSettings)asset;
                        if (importSettings != BlenderImporterGlobalSettings.instance)
                        {
                            Debug.LogWarning(
                                "Only one instance of BlenderImporterGlobalSettings is allowed! Deleting the duplicate.");
